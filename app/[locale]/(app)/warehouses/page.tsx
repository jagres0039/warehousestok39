...
export default async function WarehousesListPage({ params, searchParams }: PageProps) {
      ...
      <PageHeader
        ...
        action={
          canMutate(session.role)
            ? { label: t("warehouseCreateAction"), href: `/${locale}/warehouses/new` }
            : undefined
        }
      />
              ...
              result.rows.map((w) => (
                <tr key={w.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3 text-right">
                    {canMutate(session.role) ? (
                      <Link
                        href={`/${locale}/warehouses/${w.id}/edit`}
